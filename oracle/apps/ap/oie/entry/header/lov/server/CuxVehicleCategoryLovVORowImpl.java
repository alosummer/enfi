package cux.oracle.apps.ap.oie.entry.header.lov.server;

import oracle.apps.fnd.framework.server.OAViewRowImpl;

import oracle.jbo.server.AttributeDefImpl;
// ---------------------------------------------------------------------
// ---    File generated by Oracle ADF Business Components Design Time.
// ---    Custom code may be added to this class.
// ---    Warning: Do not modify method signatures of generated methods.
// ---------------------------------------------------------------------
public class CuxVehicleCategoryLovVORowImpl extends OAViewRowImpl {
    public static final int FLEXVALUE = 0;
    public static final int DESCRIPTION = 1;
    public static final int SUMMARYFLAG = 2;
    public static final int PARENTFLEXVALUELOW = 3;

    /**This is the default constructor (do not remove)
     */
    public CuxVehicleCategoryLovVORowImpl() {
    }

    /**Gets the attribute value for the calculated attribute FlexValue
     */
    public String getFlexValue() {
        return (String)getAttributeInternal(FLEXVALUE);
    }

    /**Sets <code>value</code> as the attribute value for the calculated attribute FlexValue
     */
    public void setFlexValue(String value) {
        setAttributeInternal(FLEXVALUE, value);
    }

    /**Gets the attribute value for the calculated attribute Description
     */
    public String getDescription() {
        return (String)getAttributeInternal(DESCRIPTION);
    }

    /**Sets <code>value</code> as the attribute value for the calculated attribute Description
     */
    public void setDescription(String value) {
        setAttributeInternal(DESCRIPTION, value);
    }

    /**Gets the attribute value for the calculated attribute SummaryFlag
     */
    public String getSummaryFlag() {
        return (String)getAttributeInternal(SUMMARYFLAG);
    }

    /**Sets <code>value</code> as the attribute value for the calculated attribute SummaryFlag
     */
    public void setSummaryFlag(String value) {
        setAttributeInternal(SUMMARYFLAG, value);
    }

    /**Gets the attribute value for the calculated attribute ParentFlexValueLow
     */
    public String getParentFlexValueLow() {
        return (String)getAttributeInternal(PARENTFLEXVALUELOW);
    }

    /**Sets <code>value</code> as the attribute value for the calculated attribute ParentFlexValueLow
     */
    public void setParentFlexValueLow(String value) {
        setAttributeInternal(PARENTFLEXVALUELOW, value);
    }

    /**getAttrInvokeAccessor: generated method. Do not modify.
     */
    protected Object getAttrInvokeAccessor(int index, 
                                           AttributeDefImpl attrDef) throws Exception {
        switch (index) {
        case FLEXVALUE:
            return getFlexValue();
        case DESCRIPTION:
            return getDescription();
        case SUMMARYFLAG:
            return getSummaryFlag();
        case PARENTFLEXVALUELOW:
            return getParentFlexValueLow();
        default:
            return super.getAttrInvokeAccessor(index, attrDef);
        }
    }

    /**setAttrInvokeAccessor: generated method. Do not modify.
     */
    protected void setAttrInvokeAccessor(int index, Object value, 
                                         AttributeDefImpl attrDef) throws Exception {
        switch (index) {
        case FLEXVALUE:
            setFlexValue((String)value);
            return;
        case DESCRIPTION:
            setDescription((String)value);
            return;
        case SUMMARYFLAG:
            setSummaryFlag((String)value);
            return;
        case PARENTFLEXVALUELOW:
            setParentFlexValueLow((String)value);
            return;
        default:
            super.setAttrInvokeAccessor(index, value, attrDef);
            return;
        }
    }
}
